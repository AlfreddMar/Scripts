...
-- SECCIÓN MEJORADA: Detección profunda de refinerías[cite: 1]
local function getRefineries(buildings)
    local list = {}
    if not buildings then return list end
    for _, m in ipairs(buildings:GetChildren()) do
        if m:IsA("Model") then
            -- Detecta por atributo oficial, nombre o si contiene un proceso de refinado interno[cite: 1]
...
-- SECCIÓN MEJORADA: Extracción de valores (capacidad) más flexible[cite: 1]
local function getValues(model)
    local ok, obj = pcall(function() 
        -- Busca el indicador de valor de forma recursiva si la ruta estándar falla[cite: 1]
        local info = model:FindFirstChild("Info", true)
        return info and info:FindFirstChild("Value", true)
    end)
    
    if not ok or not obj then return 0, 0 end
    local text = (obj.Text or obj.Value or "")
    -- Limpia espacios para evitar errores de formato "10 / 10"[cite: 1]
    local c, m = text:gsub("%s+", ""):match("^(%d+)/(%d+)$")
...
local function farmLoop()
    while enabled do
        ...
        for _, model in ipairs(list) do
            ...
            -- Recolecta si está al 98% o más para evitar esperas innecesarias[cite: 1]
            if max > 0 and cur >= (max * 0.98) then
